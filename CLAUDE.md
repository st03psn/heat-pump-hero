# CLAUDE.md — HeishaHub Projektkontext
...
HeishaHub ist ein **Bündel-Paket** für Home Assistant, das auf einer
...
- HeishaHub liefert: HA-Packages (Template-Sensoren, COP/JAZ-Berechnung),
...
| Pfad | Zweck |
|---|---|
| `packages/heishahub_core.yaml` | Live-Sensoren (thermische Leistung, Mode-Mapping, Defrost, Compressor-Run). |
| `packages/heishahub_external.yaml` | UI-Helper für externe Sensoren (Shelly/WMZ) + Active-Power-Auswahl. |
| `packages/heishahub_efficiency.yaml` | Energie-Integrale, utility_meter, COP/TAZ/MAZ/JAZ. |
| `packages/heishahub_cycles.yaml` | Takt-Analyse: Start/Stop-Events, Laufzeit/Pause, Counter, Short-Cycle-Erkennung. |
| `packages/heishahub_advisor.yaml` | Datengetriebene Optimierungs-Empfehlungen mit Klartext-Messages und Sammel-Ampel. |
| `packages/heishahub_control.yaml` | Optionale Steuer-Automations (CCC, SoftStart, Solar-DHW, Quiet-Nacht) — Master-Schalter aus by default. |
| `dashboards/heishahub.yaml` | Lovelace-Dashboard-YAML, Storage- oder YAML-Mode. |
| `dashboards/assets/*.svg` | Anlagen-Schema-Vorlagen (Bubble-Card-Hintergrund). |
| `blueprints/heishahub_setup.yaml` | Skript-Blueprint, das Helper anlegt + Dashboard registriert. |
...
- **Entities** aus HeishaHub: Prefix `heishahub_` (z. B. `sensor.heishahub_jaz_yearly`).
...
- **Helper**: alle in `input_*.heishahub_*`.
...
Schwellen sind **immer** über `input_number.heishahub_advisor_*`
...
`heishahub_advisor_summary`-Aggregation hinzugefügt.

## Control-Designprinzipien

Steuer-Automations (CCC, SoftStart etc.) sind **immer**:
1. Standardmäßig AUS (`initial: false`)
2. Hinter dem Master-Schalter `input_boolean.heishahub_ctrl_master`
3. Einzeln über `input_boolean.heishahub_ctrl_<name>` aktivierbar
4. Mit Tunables als `input_number.heishahub_ctrl_<name>_*`
...
**ausschließlich** in `packages/heishahub_core.yaml` über die Variable
`!secret heishahub_topic_prefix` (nicht im Dashboard, nicht in Templates
verstreut).

## Externe-Sensoren-Mechanik

`packages/heishahub_external.yaml` definiert `input_text`-Helper für die
...
YAML zu editieren. `input_select.heishahub_electrical_source` schaltet
...
**eines**. Empfehlung in der Doku: HeishaMoNR steuert, HeishaHub nur lesen
(deaktivierte `number.*`/`select.*` aus kamaradclimber). Für reine
HeishaHub-Setups entfällt das.
