...
Ports hph_export.yaml automations:
  - hph_export_daily    (03:00, if schedule == daily)
  - hph_export_weekly   (03:00 Monday, if schedule == weekly)
  - hph_export_monthly  (03:00 on 1st, if schedule == monthly)
...
async def async_setup(hass: HomeAssistant, entry: ConfigEntry) -> list[Callable]:
    ...
    def _export_trigger(now: datetime) -> None:
        ...
        if schedule == "daily":
            should_fire = True
        elif schedule == "weekly" and now.weekday() == 0:
            should_fire = True
